import { supabase } from "@/lib/supabase";import { countryCodes } from "@/data/countryCodes";import { calculateScore } from "@/lib/scoring";

type Slot = {label: string;team: string;};

type Match = {id: number;teamA: Slot;teamB: Slot;};

type ActualResult = {match_id: number;winner: string;};

type ActualGroupRanking = {group_name: string;position: number;team: string;};

function Flag({ team }: { team: string }) {const code = countryCodes[team];

if (!code) return ;

return (<imgsrc={https://flagcdn.com/w40/${code}.png}alt={${team} flag}className="h-5 w-7 rounded object-cover shadow"/>);}

function TeamLabel({ team }: { team: string }) {if (!team) return TBD;

return ({team});}

function TeamRow({ team, index }: { team: string; index: number }) {return ({index + 1}
...
function ReadOnlyMatch({match,pickedWinner,}: {match: Match;pickedWinner?: Slot;}) {const pickedTeam = pickedWinner?.team;
...
export default async function BracketPage({params,}: {params: Promise<{ id: string }>;}) {const { id } = await params;
...
const { data: actualGroupRankings } = await supabase.from("actual_group_rankings").select("*");

const { data: actualThirdPlaceTeams } = await supabase.from("actual_third_place_teams").select("*");if (error || !data) {return (Bracket not found);}

const bracket = data.data;const rankings = bracket.rankings || {};const thirdPlaceRanking = bracket.thirdPlaceRanking || [];const winnersByMatch = bracket.winnersByMatch || {};const champion = winnersByMatch["104"];

const score = calculateScore(bracket,(actualResults || []) as ActualResult[],(actualGroupRankings || []) as ActualGroupRanking[],actualThirdPlaceTeams || []);

function getTeam(group: string, position: number): string {return (rankings[group] as string[] | undefined)?.[position - 1] || "";}

function getThirdPlaceGroup(team: string): string {const entry = Object.entries(rankings).find(([, groupRanking]) => (groupRanking as string[])[2] === team);

return entry?.[0] || "";

}

function getThirdPlaceSlot(allowedGroups: string[],usedGroups: Set): Slot {for (const team of (thirdPlaceRanking as string[]).slice(0, 8)) {const group = getThirdPlaceGroup(team);
...
function buildRoundOf32(): Match[] {const usedThirdGroups = new Set();const third = (allowed: string[]) =>getThirdPlaceSlot(allowed, usedThirdGroups);
...
function winner(matchId: number): Slot {return winnersByMatch[String(matchId)] || { label: W${matchId}, team: "" };}

const round32 = buildRoundOf32();

const round16: Match[] = [{ id: 89, teamA: winner(74), teamB: winner(77) },{ id: 90, teamA: winner(73), teamB: winner(75) },{ id: 93, teamA: winner(83), teamB: winner(84) },{ id: 94, teamA: winner(81), teamB: winner(82) },{ id: 91, teamA: winner(76), teamB: winner(78) },{ id: 92, teamA: winner(79), teamB: winner(80) },{ id: 95, teamA: winner(86), teamB: winner(88) },{ id: 96, teamA: winner(85), teamB: winner(87) },];

const quarterfinals: Match[] = [{ id: 97, teamA: winner(89), teamB: winner(90) },{ id: 98, teamA: winner(93), teamB: winner(94) },{ id: 99, teamA: winner(91), teamB: winner(92) },{ id: 100, teamA: winner(95), teamB: winner(96) },];

const semifinals: Match[] = [{ id: 101, teamA: winner(97), teamB: winner(98) },{ id: 102, teamA: winner(99), teamB: winner(100) },];

function loser(matchId: number, teamA: Slot, teamB: Slot): Slot {const pickedWinner = winner(matchId);
...
const thirdPlace: Match = {id: 103,teamA: loser(101, semifinals[0].teamA, semifinals[0].teamB),teamB: loser(102, semifinals[1].teamA, semifinals[1].teamB),};

const final: Match = {id: 104,teamA: winner(101),teamB: winner(102),};
